...
bc1 = fem.dirichletbc(u_D, fem.locate_dofs_topological(V, fdim, boundary_facets), V)

def top_boundary(x):
    return np.isclose(x[1], 1)

fdim = domain.topology.dim - 1
boundary_facets = mesh.locate_entities_boundary(domain, fdim, top_boundary)

u_D = np.array([0,-0.3,0], dtype=ScalarType)
bc2 = fem.dirichletbc(u_D, fem.locate_dofs_topological(V, fdim, boundary_facets), V)
